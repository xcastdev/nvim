...
-- NOTE:Disabled for tmux-nvim-navigator
  ...
  { -- LSP Configuration & Plugins
    ...
    config = function()
      ...
      local servers = {
        ...
        pyright = {
          handlers = {
            -- Allows unused variables, parameters, etc that begin with a '_'
            ['textDocument/publishDiagnostics'] = function(err, results, ctx)
              local new_index = 1
              local size_orig = #results.diagnostics
              for _, diagnostic in ipairs(results.diagnostics) do
                if not string.match(diagnostic.message, '"_.+" is not accessed') then
                  results.diagnostics[new_index] = diagnostic
                  new_index = new_index + 1
                end
              end
              for i = new_index, size_orig do
                results.diagnostics[i] = nil
              end
              vim.lsp.diagnostic.on_publish_diagnostics(err, results, ctx, {})
            end,
          },
